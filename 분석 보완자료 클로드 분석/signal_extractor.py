...
from core.stock_resolver import resolve_symbol  # ✅ 추가
...
def _get_event_date(content: IntelContent) -> Optional[str]:
    """
    Signal event_date 결정 우선순위:
    1. published_at  — YouTube 영상 게시일 (실제 정보 생성 시점)
    2. analyzed_at   — 분석 실행일
    3. created_at    — DB 생성일
    4. 오늘

    ⚠️ analyzed_at 만 쓰면 3일 전 영상을 오늘 분석했을 때
       Signal.event_date 가 오늘로 찍혀 차트 날짜 매칭 오차 발생.
    """
    # 1순위: 영상/뉴스 실제 게시일
    ...
    if isinstance(mentioned, list):
        for m in mentioned:
            ...
            if not name or name in saved_names:
                continue
            # ✅ BUG FIX: stock_resolver 로 종목코드 적극 탐색
