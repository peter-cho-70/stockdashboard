...
신호(Signal) 집계 API
  ...
  GET  /api/intel/stocks/{symbol}/buy-score          — 매수 타이밍 스코어 ✅
  GET  /api/intel/portfolio/risk-radar               — 포트폴리오 리스크 레이더 ✅
...
# ── GET /api/intel/stocks/{symbol}/buy-score ────────────────────
...
def get_buy_score(
    ...
    """
    매수 타이밍 종합 스코어 (0~100).
    AI 추가 호출 없음 — 저장된 Signal DB 기반.
    grade: A(70+) 매수검토 / B(50+) 조건부 / C(30+) 관망 / D 보류
    """
...
# ── GET /api/intel/portfolio/risk-radar ─────────────────────────
...
def get_portfolio_risk_radar(
    ...
    """
    포트폴리오 전체 리스크 레이더 (5개 축, 0~100).
    - 섹터 집중도: 같은 섹터 종목 비율
    - 매크로 노출도: 최근 NEGATIVE 매크로 Signal 비율
    - Negative Signal: 최근 부정 종목이슈 비율
    - 미설명 급변: causeSource=none 비율 (PriceMoveCause 없는 종목)
    - 분석 커버리지: 최근 days일 내 분석 있는 종목 비율
    """
    from collections import Counter
    from config.database import MacroSignal, SectorSignal, StockIssue, PriceMoveCause, IntelContent
    ...
    # 1) 섹터 집중도 (헤르핀달 지수 → 0~100)
    from core.sector_peers import normalize_sector
    sector_counts = Counter(
        normalize_sector(s.sector, s.symbol) or "미분류"
        for s in stocks
    )
    hhi = sum((c / n) ** 2 for c in sector_counts.values()) if n > 0 else 0
    concentration = round(hhi * 100, 1)  # 0=완전분산, 100=한섹터

    # 2) 매크로 노출도 (NEGATIVE 매크로 비율)
    ...
    # 4) 미설명 급변 비율 (PriceMoveCause 없는 종목)
    stocks_with_cause = {
        r[0] for r in db.query(PriceMoveCause.stock_id)
        .filter(PriceMoveCause.event_date >= since)
        .all()
    }
    unexplained = round((1 - len(stocks_with_cause) / n) * 100, 1) if n > 0 else 100

    # 5) 분석 커버리지 (최근 days일 내 분석 있는 종목)
    from config.database import StockIssue as SI
    covered_ids = {
        r[0] for r in db.query(SI.stock_id)
        .join(IntelContent, SI.content_id == IntelContent.id)
        .filter(IntelContent.analyzed_at >= since)
        .all()
    }
    coverage = round(len(covered_ids) / n * 100, 1) if n > 0 else 0
    coverage_gap = round(100 - coverage, 1)  # 높을수록 위험
    ...
    axes = [
        {
            ...
            "description": f"최다 섹터: {sector_counts.most_common(1)[0][0]} ({sector_counts.most_common(1)[0][1]}개)",
        },
        {
            "axis": "매크로노출",
        ...
        },
        {
            ...
            "description": f"AI 원인 미검색 종목 {n - len(stocks_with_cause)}/{n}개",
        },
        {
            "axis": "분석공백",
            "value": coverage_gap,
