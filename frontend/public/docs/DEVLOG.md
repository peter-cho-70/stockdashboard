...
### Phase 7 — 놓친 시세 이력 캐치업 (2026-07-24)
- **YouTube 자막 없을 때 Gemini 직접 시청으로 전환**: 자막이 없는 영상(IP 차단 등)은 기존엔 URL 문자열만
  텍스트 프롬프트로 넘겨서 Gemini가 실제 영상을 보지 못한 채 그럴듯한 내용을 지어내는(환각) 문제가 있었음.
  `core/gemini_client.py`에 `generate_json_from_youtube` 추가 — `types.Part.from_uri`로 YouTube URL을
  media part로 첨부해 모델이 영상을 직접 시청하게 함. `core/ai_analyzer.py`의 `_extract_youtube_with_gemini`가
  자막 유무에 따라 텍스트 분석/직접 시청 경로를 분기하도록 수정.
- **가격 이력(PriceHistory) 캐치업**: 기존엔 장마감 스케줄러(15:35)가 그 시각에 떠있어야만 그날 시세를
  `PriceHistory`에 저장했는데, 컴퓨터를 며칠 꺼두면 그 사이 날짜는 이력 자체가 비어 있었고, 앱 시작 시
  자동 캐치업(`run_startup_catchup_sync`)도 최신가 1건만 받아와 중간 날짜의 실제 등락률은 영영 복원할 방법이
  없었음. `core/price_updater.py`에 `fetch_krx_price_range`(pykrx 기간 조회) + `catchup_price_history`
  (종목별 마지막 저장일 다음날 ~ 최근 거래일까지 갭을 계산해 실제 일별 OHLC·등락률로 채움) 추가. 국내(KRX)
  종목에 한정하고, 알림(급등락)은 소급 생성하지 않고 조회용 이력만 복원하기로 결정(사용자 확인).
  `run_startup_catchup_sync`에 통합해 앱 시작 시 자동 실행(KIS/키움 미설정 여부와 무관하게 항상 실행) +
  대시보드에 "놓친 시세 다시 체크" 수동 버튼도 추가(`POST /portfolio/price-history/catchup`).
- **가격 이력 캐치업 — 중간 구멍 못 채우는 버그 수정**: 실사용 중 7/20~22가 안 채워지는 문제 발견. 원인은
  `catchup_price_history`가 "종목별 마지막 저장일 다음날부터"만 봤는데, 일반 시세갱신이 앱을 켤 때마다
  "오늘 날짜"로 바로 최신 이력을 써버려서(예: 7/23·24는 이미 있음) 그보다 앞선 7/20~22 구멍이 "이미 최신
  이후"로 오인되어 영영 안 채워졌던 것. 최근 N일(기본 30일) 구간을 매번 통째로 다시 스캔해 빈 날짜를 채우는
  방식으로 변경. 같이 발견한 원인 제공 버그 — `PortfolioManager.save_price_history()`가 (다른 저장 함수들과
  달리) `date.today()` wall-clock을 그대로 써서 주말에 동기화가 한 번이라도 돌면 그 요일에 직전 거래일
  값이 중복 저장되던 것도 `get_latest_trading_date()` 기준으로 수정. `PriceHistory`에 `(stock_id, date)`
  유니크 제약 추가(`uq_price_history_stock_date`) — 시작 시 자동 캐치업과 수동 버튼이 우연히 겹쳐도
  중복 행 대신 그 종목만 롤백하고 다음 종목으로 진행. 기존에 쌓여있던 중복 126건은 직접 정리.
- **포트폴리오 스냅샷(수익) 캐치업 추가**: 시세 이력은 복원됐지만 "일별 손익" 대시보드 차트에는 여전히
  구멍이 남는다는 피드백 — `PortfolioSnapshot`(총 평가금액·손익·수익률)은 시세와 별개 테이블이라
  `catchup_price_history`가 손대지 않았기 때문. `catchup_portfolio_snapshots` 추가 — 이제 복원된
  `PriceHistory` 종가를 바탕으로, 그 날짜 이후(> D) 매매가 없었던 종목은 현재 수량·평단가를 그대로 써서
  정확히 계산하고, 매매가 있었던 종목은 이후 체결내역을 거꾸로 되돌려 수량만 복원(평단가는 근사 — 그 구간에
  매수가 있었던 종목만 살짝 부정확할 수 있음, 매도는 평단가에 영향 없어 정확). 캐치업 대상 날짜에서 주말은
  제외하도록 필터링 — 안 그러면 예전부터 남아있던 `PriceHistory`의 주말 오염 날짜(7/11·7/12 등, wall-clock
  버그 잔재)까지 그대로 믿고 가짜 스냅샷을 만들어버리는 것을 뒤늦게 발견해 수정(만들어졌던 2건은 삭제).
  `run_startup_catchup_sync`(체결내역 동기화 이후에 실행 — 매매로 바뀐 수량을 정확히 반영하려면 순서가
  중요)와 수동 버튼(`POST /portfolio/price-history/catchup`) 양쪽에 통합, 응답 메시지에 "수익 이력 N일치"도
  함께 표시.
- **주말 날짜 오염 데이터 전량 정리**: 사용자 요청으로 `PriceHistory`·`PortfolioSnapshot`에 남아있던 옛
  wall-clock 버그 잔재를 전부 확인 후 삭제 — `PriceHistory` 5/30·5/31·6/21·7/11·7/12(종목 수만큼, 총 153건),
  `PortfolioSnapshot` 5/30·5/31·6/21(3건). 삭제 전 각각 인접 평일과 값이 완전히 동일한 것을 확인해 안전하게
  판단(직전 거래일 값이 그대로 복제된 패턴). 정리 중 새로 발견한 별개 이슈 — 6/2·6/3(둘 다 평일)도 값이
  완전히 동일한데, 주말이 아니라서 이번 정리 대상에서는 제외하고 [STATUS](STATUS.md) 알려진 이슈에 기록만
  해둠(우연의 일치인지 다른 버그 변종인지 미확인, 보류).

### Phase 8 — 체결내역 종목별 집계 현재가 표시 + 캐치업 스코프 정리 (2026-07-31)
- **체결내역 "종목별 집계"에 현재가·등락률 참고 표시**: `/portfolio/trades`의 종목별 집계 표는 매수 체결만
  합산해 시세 정보가 전혀 없었음. 보유 중인 종목은 `GET /portfolio/stocks` 결과를 그대로 매칭해서 채우고,
  이미 전량 매도해 보유 목록에 없는 종목은 `POST /portfolio/stocks/{symbol}/ensure`(차트 미리보기용 등록)로
  개별 조회해 보완 — 동시 4개씩 배치 처리. 단가 대비 등락률을 한국 증시 관례(상승 빨강/하락 파랑)로 같이
  표시. 순수 참고용이라 평가손익까지는 계산하지 않음(`buyQty`가 실제 잔여 수량과 다를 수 있어 부정확할 수
  있기 때문).
- **캐치업 대상 종목 스코프 축소(버그 예방)**: 위 기능이 `ensure_stock_for_chart`를 호출하면 그 종목이
  `Stock.is_active=True`로 영구 등록되는데, Phase 7의 `catchup_price_history`/`catchup_portfolio_snapshots`가
  `is_active` 종목 전체를 스캔 대상으로 삼고 있어서, 옛 매도 종목을 조회할수록 앱을 켤 때마다 도는 pykrx
  스캔 대상이 계속 불어나는 구조적 문제가 있었음(아직 실제 데이터는 오염되지 않았지만 사용 패턴상 필연적으로
  발생). `core/price_updater.py`에 `_tracked_stocks(db, krx_only)` 헬퍼를 추가해 "현재 보유 중(qty>0) 이거나
  실제 매매 이력(`portfolio_trades`)이 있는 종목"만 캐치업 대상으로 한정 — 차트 미리보기·섹터 묶어보기 등으로만
  등록된 조회용 종목은 제외. 포트폴리오 스냅샷 복원은 해외주식 총평가금액도 필요해 시장 제한 없이, 가격 이력
  캐치업(pykrx 호출)은 국내(KRX)로만 한정.
- **캐치업 쿨다운 추가**: `catchup_price_history`가 앱 시작마다(로컬 개발 중 재시작이 빈번함) 매번 전 종목
  30일 재스캔을 돌던 것을, 마지막 실행 시각을 `backend/data/price_catchup_state.json`에 남겨 30분
  (`CATCHUP_COOLDOWN_MINUTES`) 이내 재실행은 건너뛰도록 변경. 대시보드 "놓친 시세 다시 체크" 수동 버튼은
  `force=True`로 호출해 쿨다운과 무관하게 항상 실행.
