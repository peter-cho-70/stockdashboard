...
> **2026-07-11 업데이트 — 초기버전 구현 현황**
> 이 문서 작성 이후 실제 코드를 다시 파본 결과:
> - **Phase 1(Signal 적중률 추적)·Phase 2(Lead-Lag 분석)는 이미 구현되어 있었다** (`core/signal_tracker.py`, `core/lead_lag.py`) —
>   다만 API만 있고 프론트엔드에 연결이 안 되어 있었음.
> - **결정적 버그 발견**: `core/ai_analyzer.py`의 분석 프롬프트가 6/2부터 `macro_analysis`/`sector_analysis`의 내부
>   스키마(`sentiment`/`outlook`/`impact` 필드)를 생략 표기(`[...]`)로만 써놔서, AI가 해당 필드를 빼고 반환 →
>   `signal_extractor.py`가 조용히 스킵 → 5주간 `macro_signals` 0건, `sector_signals`는 96%가 기본값 NEUTRAL로
>   쌓임. **이번에 프롬프트를 수정**했고, 6/2~7/11 구간은 백필하지 않기로 결정(비용 발생하는 재분석 생략, 새로
>   들어오는 데이터부터 정상화).
> - **Phase 3(변동성 스코어)를 완성**해 매수스코어(A/B/C/D) × 변동성(LOW/MEDIUM/HIGH) 매트릭스를 구현
>   (`core/volatility_forecast.py`, `GET /api/intel/stocks/{symbol}/forecast`).
> - 이미 계산되지만 프론트에 연결 안 됐던 Signal 적중률·Lead-Lag·리스크 레이더를 인텔리전스 허브
>   "예측·리스크" 탭으로 새로 노출.
> - **2026-07-11 추가 업데이트 — Phase 4(패턴 라이브러리) 완료**: `macro_signals`가 프롬프트 버그 수정 후
>   397건으로 늘어나면서(감성도 다양) 패턴 검증에 충분한 데이터가 쌓여 착수. `core/pattern_library.py`가
>   매크로 이슈×섹터 반응을 실측 집계(`GET /intel/patterns`), 고신뢰 패턴은 신규 Signal 발생 시 자동 알림.
> - **2026-07-11 추가 업데이트 — Phase 5(AI Provider 스코어카드) 완료**: 문서의 pseudo-code가 가정한
>   `IntelContent.analysis_provider`는 실제론 없어서 신설(기존엔 어떤 AI로 분석했는지 전혀 기록 안 됐음).
>   `core/provider_scorecard.py`가 provider별 Signal 적중률 집계(`GET /intel/providers/accuracy`), 표본
>   10건 이상 중 최고 적중 provider를 명시적 지정 없는 호출에 한해 자동 사용. 과거 데이터엔 소급 적용 불가라
>   지금은 표본 0건 — 새로 분석되는 콘텐츠부터 쌓임.
> - **2026-07-11 추가 업데이트 — Phase 6·7·8 완료 (전체 로드맵 완료)**:
>   - **Phase 8(섹터 로테이션 감지)**: `core/sector_rotation.py`가 최근 window_days를 전·후반기로 나눠
>     섹터별 SectorSignal 평균 감성 점수 변화(delta)를 계산, 상승/하락 섹터 Top3와 보유 종목 편중 경고를
>     제공(`GET /intel/sector-rotation`). 실측 결과: 2차전지가 +0.67→-0.22(급락), 자동차가 -0.10→+0.33(반등).
>   - **Phase 7(시나리오 시뮬레이터)**: 문서 원안의 하드코딩 `MACRO_SCENARIOS` 대신, Phase 4에서 이미 실측한
>     `pattern_library`의 avg_move_pct·hit_rate를 그대로 재사용하도록 설계를 바꿈 — 새 테이블도, 지어낸 숫자도
>     없음. `core/scenario_simulator.py`가 트리거(topic×sentiment)를 보유 종목 섹터에 매칭해 비중 가중합산
>     추정 등락률/손익을 계산(`GET /intel/portfolio/simulate`).
>   - **Phase 6(투자 가설 추적)**: `investment_theses` 테이블 신설. `core/thesis_tracker.py`가 활성 가설을
>     최근 7일 Signal(카테고리별 매크로/섹터/종목)과 대조해 지지(POSITIVE)/반박(NEGATIVE)을 집계, 표본 3건
>     이상에서 지지율 80% 이상이면 confirmed·20% 이하면 invalidated로 자동 전환(매일 16:00 스케줄러 +
>     수동 실행 API). 인텔리전스 허브에 "투자 가설" 탭 신설, 실제 가설 1건("HBM 수요 확대 → 반도체 업황
>     개선") 등록·검증까지 완료(지지 42 · 반박 9 · 82% → confirmed).
